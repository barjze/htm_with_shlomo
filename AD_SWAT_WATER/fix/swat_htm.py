...
def runner(input_data,parameters):
    ...
    # --- ensure valid encoder range (avoid min == max) ---
    if config['channel_type'] == 0:  # analog
        if config.get('diff_enabled', False):
            mn, mx = _safe_diff_range(V1EncoderParams.minimum, V1EncoderParams.maximum)
        else:
            # רוחב ברירת־מחדל קטן אך לא אפס; אפשר להרחיב מעט לפי הדאטה
            default_width = max(1.0, 0.05 * abs(stage1_data['value']['max'] - stage1_data['value']['min']))
            mn, mx = _safe_range(V1EncoderParams.minimum, V1EncoderParams.maximum, default_width=default_width)
        V1EncoderParams.minimum, V1EncoderParams.maximum = mn, mx
    ...
    if config['channel_type'] == 0:
        ...
        denom = int(V1EncoderParams.maximum - V1EncoderParams.minimum + 1)
        if denom <= 0:
            denom = 1  # הגנה ממקרה קצה
        active_bits = max(1, int(sdr_size / denom))
    ...
    tm = TemporalMemory(
        ...
        maxSynapsesPerSegment     = min(tmParams["maxSynapsesPerSegment"], 255)
    )
    ...
    if hasattr(tm, "set_required_columns_for_prediction"):
    ...
    for count, record in enumerate(records):
       ...
       # diff values
        ...
        if encoding_map_idx:
            ...
            mapped = [int(encoding_map[encoding_map_idx][int(i)]) for i in default_encoding.sparse]
            _set_sdr_sparse(val1_encoding, mapped)
        else:
        ...
        if run_tm:
            ...
            cells_per_col = tmParams["cellsPerColumn"]
            predicted = np.unique(pred_cells.sparse // cells_per_col).astype(np.int32)

            predicted_sdr = SDR(encoding.dimensions)
            predicted_sdr.sparse = predicted

            enc_info.addData(encoding)
            if val_is_black:
                tm.compute(encoding, learn=False)
            else:
                tm.compute(encoding, learn=learn)
            ...
            if val_is_black:
                ...
                anomaly[count] = swat_utils.computeAnomalyScore(encoding, predicted_sdr)
                anomalyProb[count] = anomaly_history.anomalyProbability(v1_val, anomaly[count])

            if replay_buffer and tm.anomaly:
                for replay_enc in sdr_rbuffer:
                    tm.compute(replay_enc, learn=False)
        else:
            anomaly[count] = 0.0
            anomalyProb[count] = 0.0
...
def profiler_stage3(input_data,parameters):
    ...
    if config['channel_type'] == 0:
        ...
        denom = int(V1EncoderParams.maximum - V1EncoderParams.minimum + 1)
        if denom <= 0:
            denom = 1  # guard against zero/negative range
        active_bits = max(1, int(sdr_size / denom))
        V1EncoderParams.activeBits = active_bits
        sdr_sparsity = float(V1EncoderParams.activeBits / sdr_size)
...
def _set_sdr_sparse(sdr, indices):
    ...
    valid = [i for i in indices if 0 <= i < sdr.size]
...
def _safe_range(vmin, vmax, default_width=1.0):
    """
    Ensure encoder gets a non-degenerate range.
    If vmin >= vmax, expand a small artificial window around the midpoint.
    """
    if vmin is None or vmax is None:
        return (-0.5*default_width, 0.5*default_width)
    if vmax <= vmin:
        mid = 0.5 * (vmin + vmax)
        half = max(default_width * 0.5, 1e-6)
        return (mid - half, mid + half)
    return (vmin, vmax)

def _safe_diff_range(dmin, dmax):
    """
    Specifically for diff-encoding: if the diff is constant (e.g., 0),
    provide a small symmetric window to avoid min==max.
    """
    if dmax <= dmin:
        return (-0.5, 0.5)
    return (dmin, dmax)
