...
def _tm_predicted_columns(tm):
    """Return unique predicted column indices derived from predictive cells.
    Works across htm.core bindings; falls back safely if attributes are missing."""
    import numpy as np
    try:
        pc = tm.getPredictiveCells()
    except Exception:
        return np.array([], dtype=np.int32)
    # Determine cells per column
    cells_per_col = None
    for attr in ("cellsPerColumn", "cells_per_column", "cells_per_col"):
        if hasattr(tm, attr):
            try:
                val = getattr(tm, attr)
                if isinstance(val, (int, np.integer)) and val > 0:
                    cells_per_col = int(val)
                    break
            except Exception:
                pass
    if cells_per_col is None:
        # Try to infer from API; if not available, use a conservative default (32)
        try:
            # Some bindings expose numberOfCells/numberOfColumns
            ncols = tm.numberOfColumns()
            try:
                ncells = tm.numberOfCells()
                if ncols:
                    cells_per_col = int(max(1, ncells // ncols))
            except Exception:
                pass
        except Exception:
            pass
    ...
    if pc.size == 0:
        return np.array([], dtype=np.int32)
    cols = np.unique((pc // cells_per_col).astype(np.int32))
    return cols
...
def runner(input_data,parameters):
    ...
    for count, record in enumerate(records):
       ...
       # diff values
        ...
        # (removed) TM has no make_current_network_permanent(); learning is controlled via learn flag.
        ...
        if run_tm:
            ...
            if val_is_black:
                ...
                predicted = _tm_predicted_columns(tm)
            ...
            if replay_buffer and anomaly[count] > 0.0:
